...
import {
  ...
  MapPin, GraduationCap, Users, ChevronDown, X
} from 'lucide-react';
...
const emptyVacancy: Partial<JobVacancy> = {
  title: '', branch: '', workType: '', requiredGender: '',
  requiredAgeMin: null, requiredAgeMax: null, requiredQualification: '',
  requiredExperienceYears: null, requiredSkills: '', responsibilities: '',
  drivingLicenseRequired: false, vacancyCount: 1, startDate: '', endDate: '',
};
...
export default function Vacancies() {
  ...
  const handleSave = async () => {
    ...
    if (formData.startDate && formData.endDate && formData.startDate > formData.endDate) {
      setFormError('تاريخ البداية يجب أن يكون قبل تاريخ النهاية'); return;
    }
  ...
  const handleStatusChange = async (id: number, status: 'Closed' | 'Archived') => {
    try {
    ...
    <div className="h-full overflow-y-auto p-6" dir="rtl">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-4 mb-6 flex flex-wrap items-center gap-3">
        ...
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input
            type="text"
            placeholder="بحث بالرقم أو الإسم..."
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200">
                  ...
                  <th className="px-4 py-3 text-center font-semibold text-slate-600">العدد</th>
              ...
              <tbody>
                {vacancies.map((v, idx) => (
                  <tr key={v.id} className={`border-b border-slate-100 hover:bg-sky-50/40 transition-colors ${idx % 2 === 1 ? 'bg-slate-50/30' : ''}`}>
                    ...
                    <td className="px-4 py-3 text-center">
                      <span className={`px-3 py-1 rounded-full text-xs font-bold ${STATUS_COLORS[v.status]}`}>
                    ...
                    <td className="px-4 py-3 text-center">
                      <div className="flex items-center justify-center gap-1">
                        ...
                        {v.status === 'Open' && (
                          <button
                            onClick={() => handleStatusChange(v.id, 'Closed')}
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="col-span-2">
                      ...
                      <input
                        ...
                        placeholder="مثال: فني صيانة أجهزة"
                      />
                    </div>
                    <div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <select
                        ...
                        onChange={e => setField('requiredGender', e.target.value)}
                        className="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm focus:ring-2 focus:ring-sky-500 focus:border-sky-500"
                      >
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={e => setField('requiredQualification', e.target.value)}
                        className="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm focus:ring-2 focus:ring-sky-500 focus:border-sky-500"
                    ...
                    <div>
                      ...
                      <input
                        type="number"
                        value={formData.requiredAgeMin ?? ''}
                        onChange={e => setField('requiredAgeMin', e.target.value ? parseInt(e.target.value) : null)}
                        className="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm focus:ring-2 focus:ring-sky-500 focus:border-sky-500"
                      />
                    </div>
                    <div>
                      ...
                      <input
                        type="number"
                        value={formData.requiredAgeMax ?? ''}
                        onChange={e => setField('requiredAgeMax', e.target.value ? parseInt(e.target.value) : null)}
                        className="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm focus:ring-2 focus:ring-sky-500 focus:border-sky-500"
                      />
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-slate-600 mb-1">سنوات الخبرة المطلوبة</label>
                      <input
                        type="number"
                        value={formData.requiredExperienceYears ?? ''}
                        onChange={e => setField('requiredExperienceYears', e.target.value ? parseInt(e.target.value) : null)}
                        className="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm focus:ring-2 focus:ring-sky-500 focus:border-sky-500"
                      />
                    </div>
                    <div className="flex items-center gap-3 pt-5">
                      <input
                        type="checkbox"
                        id="drivingLicense"
                        checked={formData.drivingLicenseRequired || false}
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      rows={2}
                      className="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm focus:ring-2 focus:ring-sky-500 focus:border-sky-500"
                      placeholder="مثال: صيانة أجهزة تحلية، تركيب أنظمة فلترة..."
                    />
                  </div>
                  <div>
